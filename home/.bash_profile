...
alias DYL=' codex --yolo -m gpt-5.6-luna -c model_reasoning_effort=xhigh '
alias DY=' codex --yolo -m gpt-5.6-sol -c model_reasoning_effort=medium '
alias DYT=' codex --yolo -m gpt-5.6-terra -c model_reasoning_effort=high '
...
alias UUU2=' sudo systemctl restart systemd-timesyncd & ( sudo npm install -g npm && sudo npm install -g @openai/codex ) & sudo agy update & ( sudo apt update -qq && i=$(apt-cache policy claude-code | awk "/Installed/{print \$2}") && c=$(apt-cache policy claude-code | awk "/Candidate/{print \$2}") && [ "$i" != "(none)" ] && [ "$i" != "$c" ] && sudo apt install claude-code -y ) & wait '

# 溝通
- 語言：繁體中文
- 風格：簡短回覆

# 哲學
- 低耦合，明確 I/O 邊界，各模組可獨立替換
- Fail fast：異常立即報錯，禁止靜默繼續
- 契約優先：介面與型別先於實作
- 強型別，杜絕隱式轉型
- 自動化測試先行

# 硬體
- GPU：RTX 4050 6G
- AI 推論必須使用 GPU；可用時使用 CPU 屬配置錯誤，需報錯

# I/O 管理
- 三層分離：INPUT / OUTPUT / TMP，禁止跨層混寫
- 路徑以腳本位置或專案根目錄為基準；禁止硬編碼絕對路徑

# 快取／暫存隔離
> 動機：OS 碟與開發碟為不同實體硬碟，所有寫入必須留在開發碟

- 暫存、快取、中間檔一律寫入腳本目錄下的 TMP/
- 禁止寫入 ~/、/tmp、~/.cache
- __pycache__ 與 PYTHONPYCACHEPREFIX 指向 ./TMP/
- 第三方套件：① import 前設置環境變數 ② API 參數直接傳入 TMP/ 路徑

# 執行環境（Python）
- 透過 uv run 建立獨立環境，不依賴全域設定
- torch_dtype 統一使用 dtype 參數

# dev 文件規範
- 當產出供 AI 或執行者照做的文件（todo.md / plan.md / spec / runbook 等）：
  - 每步驟指定唯一實作路徑，所有決策點必須在撰寫階段給定唯一答案，禁止給選項
  - 編輯文件之前必須先溯源完整功能鏈（觸發源 → 中間傳遞 → 最終副作用），確認無邏輯斷點
  - 每個步驟必須明確說明執行後會改變什麼，副作用必須預先聲明，不得依賴執行者推斷
  - 每項目必須說明它在全鏈路中的位置與上下游接口
  - 任務完成的定義是功能鏈可通，而非步驟被執行
  - 依賴不可用時，禁止降級或自製替代，直接 fail-fast
  - 禁止 fallback / mock / 假設環境支援等降級詞彙

# 回覆規約
- 只輸出TLDR, 把它當成你要 reply 的唯一內容, 不是完整回覆之外順便附送的加分項目, 也不是不做 TLDR 處理然後直接在 reply 上表示 TLDR 這樣的天才行為.
- 使用繁體中文。
- 先檢查使用者前提是否成立，再回答。
...
- 當`自創`出現在異常位置時，可能是`字串`的 typo。
- 當`請進`出現在異常位置時，可能是`情境`的 typo。
- 當`日子`出現在異常位置時，可能是`日誌`的 typo。
...
- 當`heather`出現在異常位置時，可能是`header`的 typo。
...
- 當`階段`、`尖端`, `間斷`出現在異常位置時，可能是`簡短`的 typo。

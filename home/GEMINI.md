...
- 只做解決需求的最小變更；不過度設計、不做推測性實作；只清理自身造成的問題。
...
- 資訊不足時明確指出，不臆測；說明必要取捨。

# 需求拆解
- 按觸碰同一函式的功能聚合排序，範圍由大至小，確保開發連續性。
...
- Transformers 使用 `dtype`，不用已棄用的 `torch_dtype`。
- dtype 必須顯式指定且一致；推論優先對齊訓練 dtype，來源不明預設 BF16；不得隱式混合 dtype。

---
name: dualagy
description: "Prepare INPUT/*.md briefs for the /g/app/dual_agy planner-worker runtime backed by agy --print. Use when Codex should turn a user goal, repo state, unresolved issue, or handoff notes into low-coupling input files with explicit scope, verification, escalation, and completion criteria."
---
...
Create input documents for `/g/app/dual_agy`.

The runtime owns planner and worker invocation, JSON protocol validation, logs, and `PROGRESS/dual_agy/` state. This skill owns task strategy:
...
Keep `INPUT/*.md` short. The runtime sends every input file to each fresh planner, worker, and planner-review call. Put long context outside `INPUT/` and reference it through a Context Index.
...
For a direct one-shot fix, state that this runtime may add unnecessary overhead and provide a smaller direct handoff instead.

## Context And Progress

Add a compact Context Index when the task has more context than each call needs:
...
Record durable project facts in `STATUS.md` or another indexed file. Do not ask the planner or worker to manage `PROGRESS/dual_agy/`, runtime logs, or protocol recovery.
...
`/g/app/dual_agy` starts fresh `agy --print` calls for planner, worker, and review. It does not preserve conversation or session IDs. The runtime still persists its own resumable state under `PROGRESS/dual_agy/`.

Keep this behavior out of generated briefs unless it changes allowed scope or verification.
